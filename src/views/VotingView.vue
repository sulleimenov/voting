...
const sendVote = async (id) => {
	...
	try {
		await fetch('https://69c60516f272266f3eabc8e6.mockapi.io/voting', {
			method: 'POST',
...
<template>
	<div
		class="h-[100dvh] bg-[#020617] p-4 flex flex-col items-center justify-center overflow-hidden relative"
	>
		<div
			class="absolute -top-10 -left-10 w-64 h-64 bg-blue-500/10 blur-[80px] rounded-full"
		></div>
		<div
			class="absolute -bottom-10 -right-10 w-64 h-64 bg-emerald-500/10 blur-[80px] rounded-full"
		></div>
		...
		<div class="relative z-10 w-full max-w-sm">
			<div v-if="!voted" class="space-y-6">
				...
				<div class="grid gap-4">
					<button
						...
						:disabled="loading"
						class="relative w-full h-24 rounded-[2rem] bg-gradient-to-r text-white shadow-2xl transition-all duration-200 active:scale-95 active:brightness-110 disabled:opacity-50 overflow-hidden"
				...
				<p
					class="text-center text-white/30 text-[10px] uppercase tracking-[0.2em] pt-4"
				>
			...
			<div
				v-else
				class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[3rem] p-10 text-center shadow-2xl animate-scale-in"
			>
